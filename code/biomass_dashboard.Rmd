...
domain_biomass_map <- left_join(neon_domains_w_points, domain_biomass_summary, by = "domain_id") %>%
    ...
    text_label = str_c("<b>", domain_id, ": ",
                       domain_name,  "</b>",
                       "</br>Mean Biomass/Hour: ", mean_bm, 
                       "</br>Biomass/Hour S.D: ", sd_bm,
                       "</br># of Trap Events: ", n)
  )
...
domain_biomass_phenology <- sort_data %>% 
  ...
  mutate(
    text_label = str_c(domain_id, ": ", domain_name,
                       "\nMean Biomass/Hour: ", mean_bm, 
                       "\nBiomass/Hour S.D: ", sd_bm,
                       "\n# of Trap Events: ", n)
  )
...
leaflet(data = domain_biomass_map) %>%
  ...
  addLegend(
    ...
    pal = leaflet_palette,
    values = ~mean_bm,
    title = "Mean Biomass/Hour </br>Across Trapping Events </br>(mg)"
  )
...
domain_biomass_pheno_gg <- ggplot(data = domain_biomass_phenology) +
  ...
  labs(x = "Month", y = "Mean Biomass per Hour (mg)") +
  theme_bw() +
...
#rmarkdown::render("biomass_dashboard.Rmd", output_format = "flexdashboard::flex_dashboard")
